...
export default class VKWrapper {
    constructor({ instances, version }) {
        ...
        // this.queue = new FunctionQueue(250);
        // this.queue.eventEmitter.on("start", () => {
        //     this.run();
        // });
        // this.queue.eventEmitter.on("open", () => {
        //     this.run();
        // });
        // this.queue.start();
    ...
    run(token, queue) {
        if (this.prequeue.items.length >= 2) {
            ...
            queue.enqueue(() => {
                fetch(
                    `https://api.vk.com/method/execute?code=${executeCode}&access_token=${token}&v=${this.version}`,
                    { headers: { "Content-Type": "application/json" } }
                )
            ...
            queue.enqueue(() => {
                fetch(
                    `https://api.vk.com/method/${method}?${queryString}&access_token=${token}&v=${this.version}`,
                    { headers: { "Content-Type": "application/json" } }
                )
                    .then((res) => res.json())
                    .then((res) => {
